...
with open("./projects/example_LHCb/decompressed_output/decompressed.pickle", "rb") as handle:
    after = pickle.load(handle)
    
df_before = pd.DataFrame(before, columns=column_names)
create_branches(df_before)
before = df_before["B_mass"]
df_after = pd.DataFrame(after, columns=column_names)
...
with PdfPages("./projects/example_LHCb/plotting/analysis.pdf") as pdf:    
    ...
    counts_before, bins_before = np.histogram(
        before, bins=np.linspace(0,10000,100)
    )
    hist1 = ax1.hist(
    ...
    optimizedParameters1, pcov1 = opt.curve_fit(fit, before_bin_centers, before_bin_counts, p0=[1,4500,1000,1]);
    ...
    counts_after, bins_after = np.histogram(
        after, bins=np.linspace(0,10000,100)
    )
    hist2 = ax2.hist(
    ...
    optimizedParameters2, pcov2 = opt.curve_fit(fit, after_bin_centers, after_bin_counts, p0=[1,4500,1000,1]);
