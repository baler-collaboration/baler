...
def perform_training(config, project_path):
    ...
    print('Un-normalzing...')
    start = time.time()
    test_data_renorm = helper.renormalize(test_data,
                                          normalization_features['True min'],
                                          normalization_features['Feature Range'],
                                          config)
    reconstructed_data_renorm = helper.renormalize(reconstructed_data,
                                                   normalization_features['True min'],
                                                   normalization_features['Feature Range'],
                                                   config)
    end = time.time()
...
def perform_plotting(project_path):
    ...
    helper.plot(output_path,
                project_path+'training/before.pickle',
                project_path+'training/after.pickle')
    helper.loss_plotter(project_path+'training/loss_data.csv',
                        output_path)
...
def perform_compression(config, project_path):
    print('Compressing...')
    start = time.time()
    compressed, data_before = helper.compress(model_path=project_path+'model/model.pt',
...
def perform_decompression(config, project_path):
    print('Decompressing...')
    start = time.time()
    decompressed = helper.decompress(model_path=project_path+'model/model.pt',
                                     number_of_columns=config['number_of_columns'],
    ...
    decompressed = helper.renormalize(decompressed,
                                      normalization_features['True min'],
                                      normalization_features['Feature Range'],
                                      config)
    end = time.time()
    ...
    if config['save_as_root'] == True: ## False by default
        helper.to_root(decompressed,
                       config,
                       project_path + 'decompressed_output/decompressed.root')
        helper.to_pickle(decompressed,
